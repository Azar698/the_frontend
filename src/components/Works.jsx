  ...
  return (
    <motion.div variants={fadeIn("up", "spring", index * 0.5, 0.75)}>
      <Tilt
        ...
        className='bg-tertiary p-5 rounded-2xl sm:w-[360px] w-full hover:shadow-lg hover:shadow-primary transition-all duration-300'
      >
        <div className='relative w-full h-[230px] group'>
          ...
          <div className='absolute inset-0 flex justify-end m-3 card-img_hover opacity-0 group-hover:opacity-100 transition-opacity duration-300'>
            ...
            > 
              <LuArrowUpRight size={25} className='text-white' />
        ...
        <div className='mt-4 flex flex-wrap gap-2'>
          {tags.map((tag) => (
            <span
              key={`${name}-${tag.name}`}
              className={`text-[12px] ${tag.color} bg-tertiary-light px-2 py-1 rounded-full`}
            >
              {tag.name}
            </span>
          ))}
